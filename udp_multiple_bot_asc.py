# ######################################################################
#  @file      udp_multiple_bot_asc.py
# 
#     This file simulates 500 BotASCs connecting up to the MainASC via
#     the UDP protocol so we can measure and accurately gauge what that 
#     would require in terms of memory utilization on the MainASC's STM32
#     platform.
#  
#  @brief   
#  
...
def connect_to_mainasc(botasc_index):
    ...
    # Perhaps connect() would help us leverage the send() and recv() routines
    # instead of sendto() and recvfrom() which would have required also
    # storing the appropriate port numbers.
    socket_descriptor.connect((MAIN_ASC_IP_ADDRESS, MAIN_ASC_PORT_NUMBER + botasc_index - 1))
...
def do_communication_with_mainasc(socket_descriptor):
    while True:
        ...
        print("Message received from MainASC via UDP:=> ")
        print(mainasc_message.decode("utf-8"))
        print()
        
        socket_descriptor.send(ACK_STRING)
        
        sleep(randint(10,100)/1000.0)

print("Here! In the main udp_multiple_bot_asc program... ")
print()
...
print("Here are the UDP socket descriptor numbers: ")
print()
...
print("Main context, here after the threads; nothing to do. Awaiting the\
       multiple BotASC threads communicating continuously with the MainASC via UDP.")
print()
