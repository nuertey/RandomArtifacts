...
#define MBED_CONF_SD_INIT_FREQUENCY              400000 /*!< Initialization frequency Range (100KHz-400KHz) */
#endif
...
{
    printf("Running SDBlockDevice::_initialise_card() ... \r\n");
    ...
    if (SDCARD_V2 == _card_type) {
        printf("Indicating to SDCard that we support HC/XC capacity cards in ACMD41...\r\n");
        arg |= OCR_HCS_CCS;
    }
...
int SDBlockDevice::init()
{
    ...
    if (0 == _sectors) {
        unlock();
        printf("CMD9 failed as: \"0 == _sectors\"\r\n");
        return BD_ERROR_DEVICE_ERROR;
    }

    // Set block length to 512 (CMD16)
    if (_cmd(CMD16_SET_BLOCKLEN, _block_size) != 0) {
        printf("CMD16 [Set block length to 512] failed!\r\n");
        printf("Set %" PRIu32 "-byte block timed out\n", _block_size);
...
int SDBlockDevice::_cmd(SDBlockDevice::cmdSupported cmd, uint32_t arg, bool isAcmd, uint32_t *resp)
{
    ...
    switch (cmd)
    {
        ...
        case ACMD13_SD_STATUS:             // Response R2
        {
            ...
            // TBD Nuertey Odzeyem; special debug code:
        case CMD9_SEND_CSD:             // Response R2
        {
            // Deselect card
            _postclock_then_deselect();
            _preclock_then_select();
            auto nuertey = _spi.write(SPI_FILL_CHAR);
            auto nuerkie = _spi.write(SPI_FILL_CHAR);
            debug_if(_dbg, "Nuertey Expecting 0xFE here: 0x%" PRIx32 "\n", nuertey);
            debug_if(_dbg, "Nuerkie Expecting 0xFE here: 0x%" PRIx32 "\n", nuerkie);
            break;
        }
        default:                            // Response R1
        {
...
{
    printf("Running SDBlockDevice::_read_bytes() ... \r\n");
    uint16_t crc;
...
{
    printf("Running SDBlockDevice::_sd_sectors() ... \r\n");
...
{
    printf("Running SDBlockDevice::_wait_token() ... \r\n");
    _spi_timer.reset();
    _spi_timer.start();

    do {
        auto spiResponse = _spi.write(SPI_FILL_CHAR);
        //printf("spiResponse = 0x%02X\n", spiResponse);
        if (token == spiResponse) 
        {
            _spi_timer.stop();
            return true;
        }
    } while (_spi_timer.elapsed_time() < 50000ms);       // Wait for 300 msec for start token
    _spi_timer.stop();
